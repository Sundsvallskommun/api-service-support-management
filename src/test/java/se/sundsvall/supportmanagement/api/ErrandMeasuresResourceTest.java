...
class ErrandMeasuresResourceTest {
	...
	void updateErrandMeasure() {
		...
		when(serviceMock.updateErrandMeasure(eq(NAMESPACE), eq(MUNICIPALITY_ID), eq(ERRAND_ID), eq(MEASURE_ID), eq("\"2\""), any(Measure.class))).thenReturn(updatedMeasure);
		...
		// Verify
		verify(serviceMock).updateErrandMeasure(eq(NAMESPACE), eq(MUNICIPALITY_ID), eq(ERRAND_ID), eq(MEASURE_ID), eq("\"2\""), any(Measure.class));
		assertThat(response).isNotNull();
		assertThat(response.getId()).isEqualTo(MEASURE_ID);
	}
	...
	void deleteErrandMeasure() {
		...
		// Verify
		verify(serviceMock).deleteErrandMeasure(NAMESPACE, MUNICIPALITY_ID, ERRAND_ID, MEASURE_ID, "\"3\"");
	}
	...
	void createMeasureAttachment() {
		...
		// Arrange
		when(serviceMock.createMeasureAttachment(eq(NAMESPACE), eq(MUNICIPALITY_ID), eq(ERRAND_ID), eq(MEASURE_ID), any(MultipartFile.class)))
	...
	void linkMeasureAttachment() {
		...
		// Arrange
		when(serviceMock.linkMeasureAttachment(NAMESPACE, MUNICIPALITY_ID, ERRAND_ID, MEASURE_ID, ATTACHMENT_ID))
		...
		verify(serviceMock).linkMeasureAttachment(NAMESPACE, MUNICIPALITY_ID, ERRAND_ID, MEASURE_ID, ATTACHMENT_ID);
	}
