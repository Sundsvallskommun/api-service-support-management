...
class ErrandsHandoverResourceFailureTest {
	...
	@Test
	void handoverErrandWithInvalidNamespace() {
	...
	@Test
	void handoverErrandWithInvalidMunicipalityId() {
		final var response = webTestClient.post()
	...
	@Test
	void handoverErrandWithInvalidErrandId() {
		final var response = webTestClient.post()
	...
	@Test
	void handoverErrandWithNullBody() {
		webTestClient.post()
	...
	@Test
	void handoverErrandWithNullTarget() {
		...
		assertThat(response.getViolations())
			.extracting(Violation::field, Violation::message)
			.containsExactly(tuple("target", "must not be null"));

		verifyNoInteractions(errandServiceMock);
	}

	@Test
	void handoverErrandWithNullMapping() {
		final var request = HandoverErrandRequest.create()
			.withTarget(HandoverTarget.create().withNamespace("OTHER_NAMESPACE").withMunicipalityId("2281"));
		...
		assertThat(response.getViolations())
			.extracting(Violation::field, Violation::message)
			.containsExactly(tuple("mapping", "must not be null"));

		verifyNoInteractions(errandServiceMock);
	}

	@Test
	void handoverErrandWithBlankTargetNamespace() {
		final var request = HandoverErrandRequest.create()
			.withTarget(HandoverTarget.create().withNamespace(" ").withMunicipalityId("2281"))
			.withMapping(HandoverMapping.create().withStatus("NEW_CASE"));
		...
		assertThat(response.getViolations())
			.extracting(Violation::field, Violation::message)
			.containsExactly(tuple("target.namespace", "must not be blank"));

		verifyNoInteractions(errandServiceMock);
	}

	@Test
	void handoverErrandWithInvalidTargetMunicipalityId() {
		final var request = HandoverErrandRequest.create()
			.withTarget(HandoverTarget.create().withNamespace("OTHER_NAMESPACE").withMunicipalityId(INVALID))
			.withMapping(HandoverMapping.create().withStatus("NEW_CASE"));
		...
		assertThat(response.getViolations())
			.extracting(Violation::field, Violation::message)
			.containsExactly(tuple("target.municipalityId", "not a valid municipality ID"));
