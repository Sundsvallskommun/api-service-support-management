...
class MetadataCategoriesResourceFailureTest {
	...
	void getWithInvalidParameters(String url, Tuple expectedResponse) {
		...
		assertThat(response.getViolations()).extracting(Violation::field, Violation::message).containsExactlyInAnyOrder(expectedResponse);
	...
	void createWithInvalidNamespace() {
		...
		assertThat(response.getViolations()).extracting(Violation::field, Violation::message).containsExactlyInAnyOrder(
			tuple("createCategory.namespace", "can only contain A-Z, a-z, 0-9, - and _"));
	...
	void createWithInvalidMunicipalityId() {
		...
		final var response = webTestClient.post().uri("666/MY_NAMESPACE/metadata/categories")
			.bodyValue(ExternalIdType.create().withName("name"))
			.exchange()
			.expectStatus().isBadRequest()
			.expectBody(ConstraintViolationProblem.class)
	...
	void updateWithInvalidNamespace() {
		...
		assertThat(response.getStatus()).isEqualTo(BAD_REQUEST);
		assertThat(response.getViolations()).extracting(Violation::field, Violation::message).containsExactlyInAnyOrder(
			tuple("updateCategory.namespace", "can only contain A-Z, a-z, 0-9, - and _"));
	...
	void deleteWithInvalidNamespace() {
		...
		assertThat(response.getStatus()).isEqualTo(BAD_REQUEST);
		assertThat(response.getViolations()).extracting(Violation::field, Violation::message).containsExactlyInAnyOrder(
			tuple("deleteCategory.namespace", "can only contain A-Z, a-z, 0-9, - and _"));
