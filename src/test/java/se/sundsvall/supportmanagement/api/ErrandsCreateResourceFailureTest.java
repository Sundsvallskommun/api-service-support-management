...
class ErrandsCreateResourceFailureTest {
	...
	@Test
	void createErrandWithInvalidJsonParameterSchema() {
		// Setup
		...
		assertThat(response.getViolations())
			.extracting(Violation::field, Violation::message)
			.containsExactly(tuple("createErrand.errand.jsonParameters[0]", "Bad Request: Schema validation failed: missing required field"));
