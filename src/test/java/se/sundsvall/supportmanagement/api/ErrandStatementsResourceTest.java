...
class ErrandStatementsResourceTest {
	...
	void updateStatementJsonParameterCreated() {
		...
		when(serviceMock.updateStatementJsonParameter(eq(NAMESPACE), eq(MUNICIPALITY_ID), eq(ERRAND_ID), eq(STATEMENT_ID), any(), argThat(parameter -> KEY.equals(parameter.getKey()))))
