...
class ErrandsUpdateResourceFailureTest {
	...
	private static Errand createErrandInstance() {
		return Errand.create()
			...
			.withStakeholders(List.of(Stakeholder.create().withExternalId("cb20c51f-fcf3-42c0-b613-de563634a8ec").withExternalIdType("EMPLOYEE")))
	...
	@Test
	void updateErrandWithNullJsonParameterValue() {
		// Call
		final var response = webTestClient.patch()
			.uri(builder -> builder.path(PATH + "/{errandId}").build(Map.of("namespace", NAMESPACE, "municipalityId", MUNICIPALITY_ID, "errandId", ERRAND_ID)))
			.contentType(APPLICATION_JSON)
	...
	@Test
	void updateErrandWithEmptyJsonParameterSchemaId() {
		// Call
		final var response = webTestClient.patch()
			.uri(builder -> builder.path(PATH + "/{errandId}").build(Map.of("namespace", NAMESPACE, "municipalityId", MUNICIPALITY_ID, "errandId", ERRAND_ID)))
			.contentType(APPLICATION_JSON)
	...
	@Test
	void updateErrandWithEmptyJsonParameter() {
		// Call
		final var response = webTestClient.patch()
			.uri(builder -> builder.path(PATH + "/{errandId}").build(Map.of("namespace", NAMESPACE, "municipalityId", MUNICIPALITY_ID, "errandId", ERRAND_ID)))
			.contentType(APPLICATION_JSON)
	...
	void updateErrandWithInvalidJsonParameterSchema() {
		...
		assertThat(response.getViolations())
			.extracting(Violation::field, Violation::message)
			.containsExactly(tuple("updateErrand.errand.jsonParameters[0]", "Bad Request: Schema validation failed: missing required field"));
