...
class MetadataRoleResourceTest {
	...
	void createRole() {
		...
		webTestClient.post()
			...
			.expectHeader().location("http://localhost:".concat(String.valueOf(port)).concat(fromPath(PATH + "/{role}")
				.build(Map.of("namespace", NAMESPACE, "municipalityId", MUNICIPALITY_ID, "role", roleName)).toString()))
			.expectBody().isEmpty();
