...
class MetadataStatusResourceTest {
	...
	void createStatus() {
		...
		webTestClient.post()
			...
			.expectHeader().location("http://localhost:".concat(String.valueOf(port)).concat(fromPath(PATH + "/{status}")
				.build(Map.of("namespace", NAMESPACE, "municipalityId", MUNICIPALITY_ID, "status", statusName)).toString()))
			.expectBody().isEmpty();
