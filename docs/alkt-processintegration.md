...
public class ErrandProcess {
    ...
    private List<@NotNull ProcessSignal> awaitingSignals;  // vad processen vantar pa fran handlaggaren (5.9).
                                                    //  Alltid en lista vid lasning, tom nar ingen vantas
...
totalt, inte en per ärende. Så är det byggt i `ErrandProcessService.findLatestProcesses`: signalerna läses
bara för den senaste raden per ärende, och en sida där inget ärende har en process frågar inte efter
signaler alls. Den kostar alltså exakt det den gjorde innan fältet fanns. `GET .../processes` gör likadant,
med en fråga för ärendets alla processrader.
